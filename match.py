...
class StereoMatchApp(QMainWindow):
    ...
    def compute_f(self):
        ...
        total   = len(mask)
        print(f"F found with {inliers}/{total} inliers")
        print("F =\n", F)
        if F is None:
    ...
    def run_matching(self):
        ...
        mag = np.hypot(gx, gy)
        edge_mask = (mag > 20).astype(np.uint8)
        #edge_mask = cv2.Canny(left_rect, 10, 150)
        ...
        # Reconstruct and unwarp back to original right view
        map_y, map_x = np.indices((h,w), dtype=np.float32)
        recon_rect = cv2.remap(left_color,
