...
class StereoEpipolarApp(QMainWindow):
    def __init__(self, left_path=None, right_path=None):
        ...
        btn_reconstruct.clicked.connect(self.reconstruct_image)
    ...
    def reconstruct_image(self):
        left_edges  = self.edge_detect(self.left_gray)
        right_edges = self.edge_detect(self.right_gray)

        edge_matches = self.edge_match(left_edges, right_edges)
        if not edge_matches:
            QMessageBox.warning(self, "No Matches", "No edge matches found.")
            return

        dense_matches = self.non_edge_match(left_edges, right_edges, edge_matches)

        print(f"Found {len(edge_matches)} edge matches and {len(dense_matches)} dense matches"
              )
        all_matches = edge_matches + dense_matches

        # 3: reconstruct by warping left→right using only the calculated disparities :contentReference[oaicite:15]{index=15}
        h, w = self.left_gray.shape
        reconstructed_img = np.zeros((h, w, 3), dtype=np.uint8)
        for (xl, yl), (xr, yr) in all_matches:
            xri, yri = int(round(xr)), int(round(yr))
            if 0 <= xri < w and 0 <= yri < h:
                reconstructed_img[yri, xri] = self.left_gray[int(round(yl)), int(round(xl))]
    ...
    def edge_detect(self, image):
        ...
        # # Apply Canny and return binary edge map
        # edges = cv2.Canny(image, low_threshold, high_threshold)
        # return edges

    def edge_match(self, left_img_edge, right_img_edge):
        h, w = left_img_edge.shape
        matches = []
        win = self.patch_size // 2

        for y in range(win, h - win):
            for x in range(win, w - win):
                if left_img_edge[y, x] < 50:
                    continue
                ...
                for x2 in range(max(win, x - self.disparity_range), min(w - win, x + self.disparity_range)):
                    if b != 0:
                        y2 = int(round(-(a * x2 + c) / b))
                    else:
                        y2 = y
                    if y2 < win or y2 >= h - win:
                        continue
                    if right_img_edge[y2, x2] < 50:
                        continue
                ...
                if best_pt:
                    matches.append(((x, y), best_pt))

        return matches

    def non_edge_match(self, left_edge, right_edge, edge_matches):
        h, w = self.left_gray.shape
        win = self.patch_size // 2
        interpolated_matches = []

        # 2.2: segments = runs of non-edge pixels between two edge matches on an epipolar line :contentReference[oaicite:12]{index=12}
        segments = {}
        for (x1, y1), (x2, y2) in edge_matches:
            segments.setdefault(y1, []).append(((x1, y1), (x2, y2)))

        for y, segs in segments.items():
            segs = sorted(segs, key=lambda m: m[0][0])
            for i in range(len(segs) - 1):
                (xA, yA), (xA_p, yA_p) = segs[i]
                (xB, yB), (xB_p, yB_p) = segs[i+1]

                # 2.3: algebraic pixel‐to‐pixel mapping p' = A' + λ·(p - A) :contentReference[oaicite:13]{index=13}
                dxL = xB - xA
                dyL = yB - yA
                length_L = math.hypot(dxL, dyL)
                if length_L < 1:
                    continue

                # split “very long” segments into two subsegments :contentReference[oaicite:14]{index=14}
                subsegs = [((xA, yA), (xA_p, yA_p), (xB, yB), (xB_p, yB_p))]
                if length_L > self.max_segment_length:
                    mxL = (xA + xB) / 2
                    myL = (yA + yB) / 2
                    mxR = (xA_p + xB_p) / 2
                    myR = (yA_p + yB_p) / 2
                    subsegs = [
                        ((xA, yA), (xA_p, yA_p), (int(mxL), int(myL)), (int(mxR), int(myR))),
                        ((int(mxL), int(myL)), (int(mxR), int(myR)), (xB, yB), (xB_p, yB_p))
                    ]

                for (Ax, Ay), (Ax_p, Ay_p), (Bx, By), (Bx_p, By_p) in subsegs:
                    dx = Bx - Ax
                    dy = By - Ay
                    ...
                    lam = math.hypot(Bx_p - Ax_p, By_p - Ay_p) / seg_len

                    # interpolate every pixel in the subsegment
                    for step in range(1, int(seg_len)):
                        t = step / seg_len
                        px = Ax + dx * t
                        py = Ay + dy * t
                        ix, iy = int(round(px)), int(round(py))

                        # skip if it was an edge pixel
                        if left_edge[iy, ix] != 0:
                            continue
                        ...
                        interpolated_matches.append(((ix, iy), (px_p, py_p)))
    ...
    def compute_fundamental(self):
        ...
        if self.F is not None:
            ...
            QMessageBox.warning(self, "Error", "Failed to compute fundamental matrix")
            self.statusBar().showMessage('Failed to compute fundamental matrix')
